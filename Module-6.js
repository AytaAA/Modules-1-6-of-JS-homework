...
function filterArray(numbers, value) {
    ...
    // Пиши код ниже этой строки
     ...
     numbers.forEach(item => (item > value ? filteredNumbers.push(item) : null));
...
function getCommonElements(firstArray, secondArray) {
   ...
   firstArray.forEach(item => secondArray.includes(item)
...
const getTotalFriendCount = users => {
   return users.reduce((tot, us) => {
   return tot + us.friends.length}, 0)
};
// Пиши код выше этой строки
...
const sortedByAuthorName = [...books].sort((author1, author2) => author1.author.localeCompare(author2.author));

const sortedByReversedAuthorName = [...books].sort((author1, author2) => author2.author.localeCompare(author1.author));

const sortedByAscendingRating = [...books].sort((rat1, rat2) => rat1.rating - rat2.rating);

const sortedByDescentingRating = [...books].sort((rat1, rat2) => rat2.rating - rat1.rating);
...
const sortByAscendingBalance = users => {
   return [...users].sort((b1, b2) => b1.balance - b2.balance)
};
// Пиши код выше этой строки
...
const sortByDescendingFriendCount = users => {
   return [...users].sort((fr1, fr2) => fr2.friends.length - fr1.friends.length)
};
// Пиши код выше этой строки
...
const sortByName = users => {
   return [...users].sort((n1, n2) => n1.name.localeCompare(n2.name));
};
// Пиши код выше этой строки
...
const names = books.filter((rat) => rat.rating > MIN_BOOK_RATING).map((auth) => auth.author).sort((author1, author2) => author1.localeCompare(author2));
...
const getNamesSortedByFriendCount = users => {
  return [...users].sort((fr1, fr2) => fr1.friends.length - fr2.friends.length).map((n) => n.name)
...
const getSortedFriends = users => {
  return [...users].flatMap((n) => n.friends).sort((n1, n2) => n1.localeCompare(n2)).filter((nam, index, array) => array.indexOf(nam) === index)
...
const getTotalBalanceByGender = (users, gender) => {
  ...
  return users.filter(user => user.gender === gender).reduce((total, value) => total = total + value.balance, 0)
